  ...
  return (
    <div className="min-h-screen bg-white p-8 w-screen">
      ...
      <nav className="w-full bg-blue-600 text-white py-4">
        <div className="container mx-auto flex justify-between items-center px-6">
          <div className="space-x-4 text-lg font-bold">
            {/* <Link to="/user/createabooking" className="hover:underline text-white">Create Booking</Link> */}
            <Link to="/user/mybookings" className="hover:underline text-white">My Bookings</Link>
          </div>
        </div>
      </nav>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4 p-8 m-20">
        {/* City */}
        <div>
          <AutocompleteSelect options={centers} field={"centre"} setFormData={setFormData} formData={formData}/>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {courts.length === 0 ? (
          <p className="text-center">No courts available for the selected criteria.</p>
        ) : (
          courts.map((court) => (
            <div key={court['court Id']} className="bg-black shadow-md rounded-lg p-4">
